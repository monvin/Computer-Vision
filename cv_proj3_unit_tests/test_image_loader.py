...
from cv_proj3_code.student_code import get_fundamental_transforms, ImageLoader

def test_dataset_length():
  try:
    train_image_loader = ImageLoader(root_dir='cv_proj3_code/cv_proj3_unit_tests/small_data/', split='train', transform=get_fundamental_transforms(
        inp_size=(64, 64), pixel_mean=np.array([0.01]), pixel_std=np.array([1.001])
    ))

    test_image_loader = ImageLoader(root_dir='cv_proj3_code/cv_proj3_unit_tests/small_data/', split='test', transform=get_fundamental_transforms(
        inp_size=(64, 64), pixel_mean=np.array([0.01]), pixel_std=np.array([1.001])
    ))
  except:
    train_image_loader = ImageLoader(root_dir='../cv_proj3_code/cv_proj3_unit_tests/small_data/', split='train', transform=get_fundamental_transforms(
...
def test_load_img_from_path():
  ...
  try:
    test_image_loader = ImageLoader(root_dir='cv_proj3_code/cv_proj3_unit_tests/small_data', split='train', transform=get_fundamental_transforms(
        inp_size=(64, 64), pixel_mean=np.array([0.01]), pixel_std=np.array([1.001])
    ))
    im_path = 'cv_proj3_code/cv_proj3_unit_tests/small_data/train/bedroom/image_0003.jpg'
  except:
    test_image_loader = ImageLoader(root_dir='../cv_proj3_code/cv_proj3_unit_tests/small_data', split='train', transform=get_fundamental_transforms(
        inp_size=(64, 64), pixel_mean=np.array([0.01]), pixel_std=np.array([1.001])
    ))
  ...
  try:
    expected_data = np.loadtxt('cv_proj3_code/cv_proj3_unit_tests/test_data/sample_inp.txt')
  except:
    expected_data = np.loadtxt('../cv_proj3_code/cv_proj3_unit_tests/test_data/sample_inp.txt')
