...
def get_fundamental_transforms(inp_size: Tuple[int, int],
  ...
  return transforms.Compose([
      ...
      transforms.ToTensor(),
      transforms.Normalize(pixel_mean, pixel_std, inplace=False),
      ############################################################################
      # Student code end
      ############################################################################
  ])
...
class SimpleNet(nn.Module):
  ...
  def __init__(self):
    ...
    self.loss_criterion=loss = nn.CrossEntropyLoss(reduction='sum')
...
def get_data_augmentation_transforms(inp_size: Tuple[int, int],
  ...
  return transforms.Compose([
      ...
      ############################################################################
        ...
        torchvision.transforms.RandomResizedCrop(size=(inp_size[0], inp_size[1])),
...
class MyAlexNet(nn.Module):
  def __init__(self):
    ...
    # Step-1 Load pre-trained alexnet model
    self.between = None
    my_alexnet = alexnet(pretrained=True, progress=True)
    self.cnn_layers = my_alexnet.features
    self.fc_layers = my_alexnet.classifier
    self.between = my_alexnet.avgpool
    self.flat = nn.Sequential(torch.nn.Flatten(1,-1))
    # Step-2 Replace the output layer of fully-connected layers.
    temp = nn.Linear(in_features=4096, out_features=15, bias=True)
    temp1 = list(self.fc_layers.children())[:-1]
    temp1.append(temp)
    self.fc_layers = torch.nn.Sequential(*(temp1))
    # Step-3 Freezing the layers by setting requires_grad=False
    # example: self.cnn_layers[idx].weight.requires_grad = False
    # You could find CNN layer indices and first two FC layer indices 
    ...
    # Step-4 Assign loss
    self.loss_criterion=loss = nn.CrossEntropyLoss()
    ############################################################################
    # Student code end
    ############################################################################
  ...
  def forward(self, x: torch.tensor) -> torch.tensor:
    ...
    # Student code begin
    ############################################################################
    model_output = self.fc_layers(self.flat(self.between(self.cnn_layers(x))))
    ############################################################################
    # Student code end
    ############################################################################
...
class MyAlexNetQuantized(MyAlexNet):
  ...
  def forward(self, x: torch.tensor) -> torch.tensor:
    ...
    temp = MyAlexNetQuantized()
    model_output = temp(x).detach()
    # Step-3: Dequantize output using dequant()
       
    model_output = self.quant(model_output)
    ############################################################################
    # Student code end
    ############################################################################
...
def quantize_model(float_model: MyAlexNet,
  ...
  sample_image=None
  for sample_image, sample_label in iter(train_loader):
    quantized_model.forward(sample_image.unsqueeze(0))
